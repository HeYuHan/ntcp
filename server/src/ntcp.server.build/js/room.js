...
var Room = (function () {
    function Room() {
        ...
        this.request_dismiss_room_info = {};
        this.set_room_info = false;
    ...
    Room.prototype.Release = function (data) {
        ...
        for (var i = 0; i < this.room_players.length; i++) {
            ...
            score.nick = player.client.info.nick;
        ...
        if (free_room) {
            ...
            if (map_len > 0) {
                ...
                for (var key in player_score_map) {
                    ...
                    var sum = 0;
                    for (var k = 0; k < list.score.length; k++)
                        sum += list.score[k];
    ...
    Room.prototype.ClientBroadcast = function (client, msg) {
        for (var i = 0; i < this.m_clients.length; i++) {
            var p = this.m_clients[i];
            if (p)
                p.Send(CreateMsg(SERVER_MSG.SM_BROADCAST, {
                    uid: client.uid,
                    msg: msg
                }));
        }
    };
    Room.prototype.GameLeave = function (client, dismiss) {
        this.request_dismiss_room_info.count2 |= 1 << client.player.index;
        if (this.request_dismiss_room_info.count1 == 0) {
            this.request_dismiss_room_info.count1 |= 1 << client.player.index;
        ...
        var free = true;
        for (var i = 0; i < this.room_players.length; i++) {
            if ((this.request_dismiss_room_info.count2 & (1 << this.room_players[i].index)) == 0) {
                all_count = false;
                break;
            }
            if ((this.request_dismiss_room_info.count1 & (1 << this.room_players[i].index)) == 0) {
                free = false;
            }
        }
        if (all_count) {
